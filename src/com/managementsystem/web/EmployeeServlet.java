		...
		try{
        	...
        	case "allDeveloper":
        		listAllDeveloper(request, response);
  				break;
  			case "developerTeam":
  				if(id!=null && id!="")listDeveloperTeam(request, response,Integer.valueOf(id),employee.getEmployeeRule());
  				else listDeveloperTeam(request, response,employee.getEmployeeID(),employee.getEmployeeRule());
  				break;
  			case "team":
...
//  				break;
	...
	private void showNewEmployeeForm(HttpServletRequest request, HttpServletResponse response, int id)throws ServletException, IOException {
		request.setAttribute("id", id);
		RequestDispatcher dispatcher = request.getRequestDispatcher("employee/newDeveloper.jsp");
		dispatcher.forward(request, response);
	}
	private void showNewTeamForm(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		RequestDispatcher dispatcher = request.getRequestDispatcher("employee/newTeam.jsp");
		dispatcher.forward(request, response);
	}
	
	private void insertDeveloper(HttpServletRequest request, HttpServletResponse response, String employeeRule)throws ServletException, IOException {
		...
		int teamID=Integer.valueOf(request.getParameter("id"));
	     Employee employee=new Employee(request.getParameter("firstName"),request.getParameter("lastName"),phone,
	    		 request.getParameter("address"),randomNum,"Developer",request.getParameter("email"),
	    		 "Developer");
	...
	private void insertTeam(HttpServletRequest request, HttpServletResponse response, String employeeRule)throws ServletException, IOException {
		Random random = new Random();
		int randomNum =  random.nextInt(50000000);
		int phone= Integer.valueOf(request.getParameter("phone"));
	     Employee employee=new Employee(request.getParameter("firstName"),request.getParameter("lastName"),phone,
	    		 request.getParameter("address"),randomNum,"TeamLeader",request.getParameter("email"),
	    		 "TeamLeader");
